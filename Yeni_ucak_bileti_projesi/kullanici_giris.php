<?php
/**
 * Kullanıcı Giriş Sayfası (Normal Üye — Admin Değil)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        // Admin olmayan kullanıcıyı getir
        $stmt = $db->prepare(
            "SELECT id, ad_soyad, email, sifre FROM kullanicilar
             WHERE email = ? AND rol = 'musteri'"
        );
        $stmt->execute([$email]);
